import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";

const days = ["Thứ 2", "Thứ 3", "Thứ 4", "Thứ 5", "Thứ 6", "Thứ 7"];

const timeSlots = [
  "07:30 - 08:15", "08:15 - 09:00", "09:00 - 09:45", "10:00 - 10:45", "10:45 - 11:30",
  "13:00 - 13:45", "13:45 - 14:30", "14:30 - 15:15", "15:30 - 16:15", "16:15 - 17:00"
];

interface ScheduleBlock {
  day: number;
  startPeriod: number;
  endPeriod: number;
  courseCode: string;
  subject: string;
  classCode: string;
  teacher: string;
  room: string;
  isThucHanh: boolean;
}

const BLUE_COLOR = "bg-blue-500/15 border border-blue-500/40 text-blue-700 dark:text-blue-400";
const GREEN_COLOR = "bg-emerald-500/15 border border-emerald-500/40 text-emerald-700 dark:text-emerald-400";

const schedule: ScheduleBlock[] = [
  { day: 0, startPeriod: 1, endPeriod: 3, courseCode: "SE104.Q21", subject: "Lập trình Web", classCode: "SE104.Q21.1", teacher: "Nguyễn Văn A", room: "A301", isThucHanh: false },
  { day: 1, startPeriod: 2, endPeriod: 5, courseCode: "IT004.Q21", subject: "Cơ sở dữ liệu", classCode: "IT004.Q21.2", teacher: "Trần Thị C", room: "Lab 02", isThucHanh: true },
  { day: 3, startPeriod: 7, endPeriod: 10, courseCode: "IT003.Q21", subject: "Mạng máy tính", classCode: "IT003.Q21.1", teacher: "Phạm Thị E", room: "Lab 05", isThucHanh: true },
];

export default function StudentSchedulePage() {
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground">Thời khóa biểu sinh viên</h1>
        <p className="text-muted-foreground">Lịch học chi tiết theo kỳ</p>
      </div>

      <Card>
        <CardHeader className="pb-4">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base text-primary font-bold">Học kỳ 2, 2023-2024</CardTitle>
            <div className="flex gap-4 text-sm font-medium">
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-blue-500/40" />
                <span>Lý thuyết</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-emerald-500/40" />
                <span>Thực hành</span>
              </div>
            </div>
          </div>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-[100px_repeat(6,1fr)] gap-1 mb-2">
            <div className="p-2"></div>
            {days.map((d, i) => (
              <div key={i} className="p-2 text-center text-sm font-bold border-b-2 border-primary/20 bg-muted rounded-md">{d}</div>
            ))}
          </div>

          <div className="grid grid-cols-[100px_repeat(6,1fr)] grid-rows-[repeat(10,minmax(70px,auto))] gap-1">
            {/* [CẬP NHẬT] Hiển thị Tiết kèm mốc thời gian */}
            {Array.from({ length: 10 }).map((_, i) => (
              <div key={i} className="p-2 flex flex-col items-center justify-center rounded-md bg-muted/50 border border-transparent" style={{ gridColumn: 1, gridRow: i + 1 }}>
                <span className="text-xs font-bold text-foreground">Tiết {i + 1}</span>
                <span className="text-[10px] text-muted-foreground mt-1">{timeSlots[i]}</span>
              </div>
            ))}

            {Array.from({ length: 10 }).map((_, r) => days.map((_, c) => (
              <div key={`${r}-${c}`} className="rounded-lg border border-dashed border-border/40 -z-10" style={{ gridColumn: c + 2, gridRow: r + 1 }} />
            )))}

            {schedule.map((block, idx) => (
              <div 
                key={idx}
                className={`rounded-lg p-2 flex flex-col items-center justify-center text-center gap-0.5 z-10 transition-transform hover:scale-[1.01] ${block.isThucHanh ? GREEN_COLOR : BLUE_COLOR}`}
                style={{ 
                  gridColumn: block.day + 2, 
                  gridRow: `${block.startPeriod} / span ${block.endPeriod - block.startPeriod + 1}` 
                }}
              >
                <span className="text-[11px] font-bold leading-tight">{block.courseCode}</span>
                <span className="text-[11px] font-bold leading-tight uppercase">{block.subject}</span>
                <div className="flex flex-col text-[10px] opacity-90 mt-1">
                  <span>{block.classCode}</span>
                  <span>{block.teacher}</span>
                  <span className="font-medium mt-1">Phòng {block.room}</span>
                </div>
              </div>
            ))}
          </div>
        </CardContent>
      </Card>
    </div>
  );
}